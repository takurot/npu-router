...
- Issue、Phase、AC などのタスクごとに、最新の `main` を基点とする専用ブランチを作成する。ブランチ名は `feature/issue-<番号>-<説明>` または `feature/<phase>-<説明>` とし、複数タスクの変更を同じブランチへ混在させない。
- 複数タスクを並行して実行する場合は `git worktree` でタスクごとに作業ディレクトリとブランチを分離する。変更開始前に各 worktree のパスと現在のブランチを確認する。
