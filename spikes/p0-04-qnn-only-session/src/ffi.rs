...
//! onnxruntime v1.23.2, same commit as the `Microsoft.ML.OnnxRuntime.QNN`
//! NuGet package this spike vendors). See `../README.md` "OrtApi field
//! derivation" for how the `OrtApi` field order was derived and verified,
//! and the crate root's "Safety invariants" doc for how every call site
//! built on top of this module stays sound.
//!
//! This is a superset of the `OrtApi` prefix used by the sibling P0-03 (CPU)
//! spike: it additionally declares `AddSessionConfigEntry` (needed to set
//! `session.disable_cpu_ep_fallback=1`) and
//! `SessionOptionsAppendExecutionProvider` (needed to append the QNN EP by
//! name), both further out in the real vtable, with more padding in between.
...
pub const ONNX_TENSOR_ELEMENT_DATA_TYPE_UINT8: i32 = 2;
...
/// in declaration order, same as the sibling CPU spike). Fields this spike
/// never calls are left as `*const c_void` padding of the same count, so
/// the struct is a valid, layout-compatible prefix of the real vtable up
/// through `SessionOptionsAppendExecutionProvider` (index 216).
#[repr(C)]
pub struct OrtApi {
    ...
    _pad_101_129: [*const c_void; 29], // ReleaseCustomOpDomain .. FillStringTensorElement
    pub add_session_config_entry: unsafe extern "system" fn(
        *mut OrtSessionOptions,
        *const c_char,
        *const c_char,
    ) -> *mut OrtStatus, // 130
    _pad_131_215: [*const c_void; 85], // CreateAllocator .. ReleaseOp
    pub session_options_append_execution_provider: unsafe extern "system" fn(
        *mut OrtSessionOptions,
        *const c_char,
        *const *const c_char,
        *const *const c_char,
