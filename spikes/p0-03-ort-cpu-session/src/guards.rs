...
use crate::ffi::{
    FreeLibrary, OrtApi, OrtEnv, OrtMemoryInfo, OrtSession, OrtSessionOptions,
    OrtTensorTypeAndShapeInfo, OrtValue,
};
...
pub struct ShapeInfoGuard<'a> {
    pub api: &'a OrtApi,
    pub ptr: *mut OrtTensorTypeAndShapeInfo,
}
impl Drop for ShapeInfoGuard<'_> {
    fn drop(&mut self) {
        unsafe { (self.api.release_tensor_type_and_shape_info)(self.ptr) }
    }
}
